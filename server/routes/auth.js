...
export function authRouter(pool) {
  ...
  // Redefinir senha (somente nova senha). Requer autenticação.
  // Útil para o caso de "quero só trocar a senha do painel".
  router.post("/reset-password", requireAuth, async (req, res) => {
    ...
    // Mantemos o usuário do painel como "admin".
    const targetUsername = "admin";
    const newHash = await bcrypt.hash(String(newPassword), 12);
    await pool.query("UPDATE users SET password_hash = ? WHERE username = ?", [newHash, targetUsername]);
