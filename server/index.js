...
function resolveWritableUploadsDir() {
  const candidates = [];

  const envDir = process.env.UPLOAD_DIR;
  if (envDir) {
    candidates.push(path.isAbsolute(envDir) ? envDir : path.resolve(ROOT, envDir));
  }

  // Fallback seguro (geralmente gravável em qualquer host)
  candidates.push(path.join(os.tmpdir(), "edson_uploads"));
  ...
  for (const dir of candidates) {
    try {
      ...
      console.warn(`[EFN] Upload dir sem permissão: ${dir}. Tentando fallback...`);
    }
  }
...
const uploadsDir = resolveWritableUploadsDir();
